...
static void* g_pedFacialOverlaysStore;

// the appendable members of these structures are atArrays of 4-byte hashes. taking the
// offsets from the parser definition instead of assuming a layout keeps the mounters
// correct if a game update reorders or inserts a member, and separates out the atArrays
// that are not hash lists: the males/females/uniqueMales/uniqueFemales lists in
// CPedSkinTones hold indices into the file's own ordering, so appending them would need
// remapping.
struct ParsedHashArrays
{
	std::vector<atArray<uint32_t>*> hashes;

	// the atArrays we do not append to. only the element count is ever read from these,
	// and atArray<T>::m_count sits at the same offset for every T, so the type stands in.
	std::vector<atArray<uint8_t>*> ignored;
};

static void CollectHashArrays(rage::parStructure* def, void* base, ParsedHashArrays& out)
{
	if (def->m_baseClass)
	{
		CollectHashArrays(def->m_baseClass, base, out);
	}

	for (auto& member : def->m_members)
	{
...
static ParsedHashArrays GetHashArrays(rage::parStructure* def, void* base)
{
	ParsedHashArrays arrays;
	CollectHashArrays(def, base, arrays);
...
// CFX_PED_FACIAL_OVERLAYS and CFX_PED_SKIN_TONES both append the hash lists of a parsed
// structure to a single game-wide store of that same structure, so they share a mounter
class CfxPedMetadataMounter : public CDataFileMountInterface
{
private:
...
public:
	CfxPedMetadataMounter(const char* structName, std::function<void*()> getStore)
		: m_structName(structName), m_getStore(std::move(getStore))
	{
	}
	...
	virtual bool LoadDataFile(CDataFileMgr::DataFile* entry) override
	{
		...
		if (!store || !def || !def->m_new || !def->m_delete)
		{
			trace("%s: store or parser structure unavailable\n", m_structName);
			return false;
		}
		...
		if (dst.hashes.empty())
		{
			trace("%s: parser structure has no hash lists to append to\n", m_structName);
			return false;
		}
		...
		if (ignored > 0)
		{
			trace("%s: ignored %d entries in members that are not hash lists\n", m_structName, ignored);
		}
		return true;
	}
...
static CfxPedMetadataMounter g_pedFacialOverlaysMounter{ "CPedFacialOverlays", []() -> void* { return g_pedFacialOverlaysStore; } };
static CfxPedMetadataMounter g_pedSkinTonesMounter{ "CPedSkinTones", &GetPedSkinTonesStore };
#endif
